    ...
    if (this.modoInsertar) {
      this.clientService.addClient(empleado).subscribe({
        next: (data) => {
          ...
          this.snackBar.open('El empleado se registró correctamente', 'OK', {
            duration: 2000,
          });
        },
        error: (err) => {
          console.log(err);
          this.snackBar.open(
            'Hubo un error en el registro del empleado',
            'OK',
            { duration: 2000 }
          );
        },
      ...
      this.clientService.updateClient(empleado).subscribe({
        next: (data) => {
          this.router.navigate(['']);
          this.snackBar.open('El empleado se actualizó correctamente', 'OK', {
            duration: 2000,
          });
        },
        error: (err) => {
          console.log(err);
          this.snackBar.open(
            'Hubo un error en la actualización del empleado',
            'OK',
            { duration: 2000 }
          );
        },
